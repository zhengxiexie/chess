...
namespace idl
{
	...
	protected:
		...
		**/
		 ...
		 * rpc调用的参考签名。用户实现子类中必须必须重写这个函数
		**/
		virtual int get_qp_top_activity(const mcp_get_qp_top_activity_params &, mcp_get_qp_top_activity_response &) = 0;
		/**
		 * @brief
		 * rpc调用的参考签名。用户实现子类中必须必须重写这个函数
		**/
		virtual int get_qp_rule(const mcp_get_qp_rule_params &, mcp_get_qp_rule_response &) = 0;
		/**
		 * @brief
		 * rpc调用的参考签名。用户实现子类中必须必须重写这个函数
		**/
		virtual int get_qp_basic_info(const mcp_get_qp_basic_info_params &, mcp_get_qp_basic_info_response &) = 0;
		/**
		 * @brief
		 * rpc调用的参考签名。用户实现子类中必须必须重写这个函数
		**/
		...
		**/
		 ...
		 * rpc调用的参考签名。用户实现子类中必须必须重写这个函数
		**/
		virtual int test_redis(const mcp_test_redis_params &, mcp_test_redis_response &) = 0;
		/**
		 * @brief
		 * rpc调用的参考签名。用户实现子类中必须必须重写这个函数
		**/
		virtual int use_exchange_code(const mcp_use_exchange_code_params &, mcp_use_exchange_code_response &) = 0;
		/**
		 * @brief
		 * rpc跳转表
		**/
